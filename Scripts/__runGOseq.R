...
factor_labeling = read.table("/BiO/Research/Project1/FlapnoserayTumorTranscriptome/Resources/FlapnoseRayTranscriptome/Data/GOSeq/factor_labeling_downreg_hit_cov_abov_60_20250725.tsv", row.names=2, header=F)
...
# get gene lengths
gene_lengths = read.table("/BiO/Research/Project1/FlapnoserayTumorTranscriptome/Results/2_trinity/trinity_flapnoseray.Trinity.gene_lengths.txt", header=T, row.names=1, com='')
gene_lengths = as.matrix(gene_lengths[,1,drop=F])


# get background gene list
background = read.table("/BiO/Research/Project1/FlapnoserayTumorTranscriptome/Results/3_rsem/RSEM.gene.TMM.EXPR.matrix.filt", header=T, row.names=1)
...
GO_info = read.table("/BiO/Research/Project1/FlapnoserayTumorTranscriptome/Resources/FlapnoseRayTranscriptome/Data/GOSeq/trinotate_GO_annotation.txt", header=F, row.names=1,stringsAsFactors=F)
